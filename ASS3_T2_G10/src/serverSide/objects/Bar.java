 ...
 * everybody to eat before alerting the waiter Implementation of a client-server
 * model of type 2 (server replication). Communication is based on Java RMI.
 */
public class Bar implements BarInterface {
    ...
    /**
     * Auxiliary variable to keep track of the id of the student whose request
     * is being answered by waiter
     */
    private int studentBeingAnswered;

    /**
     * Array of booleans to keep track of the students which the waiter has
    ...
    public Bar(GeneralReposInterface reposStub, TableInterface tabStub) {
        ...
        this.studentState = new int[ExecConst.Nstudents];
     ...
     * request is putted in the queue (chef id will be N+1)
     *
     * @return chef state
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
    ...
    public synchronized int alertWaiter() throws RemoteException {
        ...
        //Update chefs state
        reposStub.setChefState(ChefState.DELIVERING_THE_PORTIONS);

        //Signal waiter that there is a pending request
        notifyAll();

        return ChefState.DELIVERING_THE_PORTIONS;
    }

    /**
     * Operation prepare the Bill
     *
     * It is called the waiter to prepare the bill of the meal eaten by the
     * students
     *
     * @return waiter state
     * @throws Remote Exception if either the invocation of the remote method,
    ...
    public synchronized int prepareBill() throws RemoteException {
        //Update Waiter state
        reposStub.setWaiterState(WaiterState.PROCESSING_THE_BILL);

        return WaiterState.PROCESSING_THE_BILL;
    }

    /**
     * Operation enter the restaurant It is called by the student to signal that
    ...
    public int enter(int studentId) throws RemoteException {
        synchronized (this) {
            ...
            studentState[studentId] = StudentState.GOING_TO_THE_RESTAURANT;
            ...
            //Update student state
            studentState[studentId] = StudentState.TAKING_A_SEAT_AT_THE_TABLE;
            reposStub.updateStudentState(studentId, studentState[studentId], true);
     ...
     *
     * It is called by the first student to arrive the restaurant to call the
     * waiter to describe the order
     *
     * @param studentId id of the student
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
     ...
     * brought signal chef that he can put request in the queue and waiter that
     * he proceed his execution to collect portions It is also used by last
     * student to arrive to signal that he wishes to pay the bill
     *
     * @param studentId id of the student
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
    ...
    public synchronized int exit(int studentId) throws RemoteException {
        ...
        numberOfPendingRequests++;
        //Update student state
        studentState[studentId] = StudentState.GOING_HOME;
     ...
     *
     * It is called by the waiter, he checks for pending service requests and if
     * not waits for them
     *
     * @return Character that represents the service to be executed 'c' : means
     * a client has arrived therefore needs to be presented with the menu by the
     * waiter 'o' : means that the waiter will hear the order and deliver it to
     * the chef 'p' : means that a portion needs to be delivered by the waiter
     * 'b' : means that the bill needs to be prepared and presented by the
     * waiter 'g' : means that some student wants to leave and waiter needs to
     * say goodbye
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
     */
