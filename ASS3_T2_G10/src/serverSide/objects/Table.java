 ...
 * waiter to bring him the bill Implementation of a client-server model of type
 * 2 (server replication). Communication is based on Java RMI.
 */
public class Table implements TableInterface {
    ...
    /**
     * Boolean variable to check if a student is informing his companion about
     * the order
     */
    private boolean informingCompanion;

    /**
     * Used to count number of students that woke up after last student to eat
     * has signalled them to
     */
    ...
    public Table(GeneralReposInterface reposStub) {
        ...
        studentsState = new int[ExecConst.Nstudents];
    ...
    public synchronized void setFirstToArrive(int firstToArrive) throws RemoteException {
        this.firstToArrive = firstToArrive;
    }
    ...
    public synchronized void setLastToArrive(int lastToArrive) throws RemoteException {
        this.lastToArrive = lastToArrive;
    }

    /**
     * Operation salute the client
     *
     * It is called by the waiter when a student enters the restaurant and needs
     * to be saluted Waiter waits for the student to take a seat (if he hasn't
     * done it yet) Waiter waits for student to finish reading the menu
     *
    ...
    public synchronized int saluteClient(int studentIdBeingAnswered) throws RemoteException {
        ...
        //Update Waiter state
        reposStub.setWaiterState(WaiterState.PRESENTING_THE_MENU);
        ...
        return WaiterState.PRESENTING_THE_MENU;
    }

    /**
     * Operation return to the bar
     *
     * It is called by the waiter to return to the bar to the appraising
     * situation state
     *
     * @return state of the waiter
     * @throws Remote Exception if either the invocation of the remote method,
    ...
    public synchronized int returnBar() throws RemoteException {
        ...
        return WaiterState.APPRAISING_SITUATION;
    }
     ...
     * first student to arrive Waiter Blocks waiting for student to describe him
     * the order
     *
     * @return state of the waiter
     * @throws Remote Exception if either the invocation of the remote method,
    ...
    public synchronized int getThePad() throws RemoteException {
        //Update Waiter state
        reposStub.setWaiterState(WaiterState.TAKING_THE_ORDER);
        ...
        return WaiterState.TAKING_THE_ORDER;
    }
    ...
    public synchronized int presentBill() throws RemoteException {
        ...
        //Update Waiter state
        reposStub.setWaiterState(WaiterState.RECEIVING_PAYMENT);
        ...
        return WaiterState.RECEIVING_PAYMENT;
    }

    /**
     * Operation siting at the table
     *
     * Student comes in the table and sits (blocks) waiting for waiter to bring
     * him the menu Called by the student (inside enter method in the bar)
     *
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
     */
    ...
    public synchronized void seatAtTable(int studentId) throws RemoteException {
        studentsState[studentId] = StudentState.TAKING_A_SEAT_AT_THE_TABLE;
    ...
    public synchronized int readMenu(int studentId) throws RemoteException {
        ...
        studentsState[studentId] = StudentState.SELECTING_THE_COURSES;
     ...
     *
     * Called by the student to begin the preparation of the order (options of
     * his companions)
     *
    ...
    public synchronized int prepareOrder() throws RemoteException {
        ...
        //Update student state
        studentsState[firstToArrive] = StudentState.ORGANIZING_THE_ORDER;
     ...
     * choose or not Blocks if not waiting to be waker up be a companion to give
     * him his preference
     *
     ...
     *
     * Called by the first student to arrive to add up a companions choice to
     * the order
     *
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
     ...
     * Blocks waiting for waiter to come with pad Wake waiter up so he can take
     * the order
     *
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
    ...
    public synchronized int joinTalk() throws RemoteException {
        //Update student state
        studentsState[firstToArrive] = StudentState.CHATTING_WITH_COMPANIONS;
        reposStub.updateStudentState(firstToArrive, studentsState[firstToArrive]);
    ...
    public synchronized int informCompanion(int studentId) throws RemoteException {
        ...
        //Update student state
        studentsState[studentId] = StudentState.CHATTING_WITH_COMPANIONS;
        reposStub.updateStudentState(studentId, studentsState[studentId]);
    ...
    public synchronized int startEating(int studentId) throws RemoteException {
        synchronized (this) {
            //Update student state
            studentsState[studentId] = StudentState.ENJOYING_THE_MEAL;
            reposStub.updateStudentState(studentId, studentsState[studentId]);
        }
     ...
     *
     * Called by the student to pay the bill Student blocks waiting for bill to
     * be presented and signals waiter when it's time to pay it
     *
     * @throws Remote Exception if either the invocation of the remote method,
     * or the communication with the registry service fails
     */
    ...
    public synchronized ReturnBoolean shouldHaveArrivedEarlier(int studentId) throws RemoteException {
        if (studentId == lastToArrive) {
            //Update student state
            studentsState[studentId] = StudentState.PAYING_THE_MEAL;
            reposStub.updateStudentState(studentId, studentsState[studentId]);
